...
export default function DropDown({ toggleDrawer }) {
    ...
    return (
        <div>
            <Button
                sx={!matches ? {cursor : "pointer", borderRadius : 1, marginX : 1, paddingX: 3, paddingY : 1, fontSize: "20px", ':hover' : { backgroundColor : "#f08a5d", color : "#fff" }} : {}}
                id="fade-button"
            ...
            >
                <Link href={"/users/event"}><MenuItem sx={{color: "#f08a5d"}} onClick={ () => matches && toggleDrawer(false) }><Button startIcon={<EventIcon/>} onClick={ () => handleClose()}>Add Event</Button></MenuItem></Link>
