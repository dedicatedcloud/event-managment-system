...
export default function Event(props) {
	...
	const handlePayment = async () => {
		const response = await fetch("/api/payment/auth", {
			method: "POST",
			body: JSON.stringify({
		...
		if (json?.token) {
			...
			const response = await fetch("/api/payment/order", {
				...
				body: JSON.stringify([{ "MerchantId": "catcos" },
				{
					"OrderNumber": Math.ceil(Math.random() * 1000),
					"OrderAmount": 50,
					"OrderDueDate": `${new Date().getDate() + 1}/${new Date().getMonth() + 1}/${new Date().getFullYear()}`,
					"OrderType": "event",
					"IssueDate": `${new Date().getDate()}/${new Date().getMonth() + 1}/${new Date().getFullYear()}`,
			...
			if (json.length > 0) {
				...
				if (_json?.message) {
					window.open(json[1].Click2Pay, "_blank");
	...
	const Form = () => {
		return activeStep === 0 ? <EventDetails next={next} eventProps={props} /> : activeStep === 1 ?
			<CustomerDetails next={next} back={backStep} props={props} /> :
			<CheckOut next={next} back={backStep} props={props} data={data} />;
	};
	...
	return (
		<Box sx={{
			...
			alignItems: "center",
			justifyContent: "center",
			height: { xs: "90rem", sm: "80rem", md: "80rem" },
			marginY: () => {
				if (activeStep === 0) {
					return "10rem"
				}
			...
			<Paper sx={{
				width: { xs: "22rem", sm: "35rem", md: "50rem", lg: "60rem" },
				paddingY: "2rem",
				paddingX: { xs: 1, sm: 2, md: "4rem", lg: "4rem" },
				borderRadius: "1rem",
				boxShadow: 6
			}}>
				...
				{matches && <MobileStepper variant="dots" position="static" activeStep={activeStep} steps={steps.length}
					sx={{
						width: "15rem",
						margin: "1rem auto",
						display: "flex",
				...
				{activeStep === steps.length ? (
					<Box component={"div"}>
						<Typography variant={"h5"} align={"center"} color={"primary"}
							gutterBottom>Confirmation</Typography>
						<Typography variant={"subtitle1"} color={"danger"} sx={{ marginY: "1rem" }}>By Clicking Confirm,
							you are affirming the validation of the Info you provided!</Typography>
						<Box component={"div"} sx={{
							...
							<Button onClick={backStep} disableElevation={true} size={"large"} variant={"contained"}
								sx={{ color: "white" }} color={"primary"}>Back</Button>
							<Button variant={"contained"} disableElevation={true} size={"large"} sx={{ color: "white" }}
								type={"submit"} onClick={() => {
									/*setIsOpen(true);*/
									handlePayment();
						...
						><Box component={"div"} sx={{
							...
							width: "70%"
								...
								<iframe style={{
									...
									borderRadius: "0.5rem",
									border: "none"
								}} id="easypay-iframe" name="easypay-iframe" src="about:blank" width="100%" height="600px" />
							</Box>
						</Backdrop>
					</Box>
				) : <Form />}
